...
class PartialParse(object):
    ...
    def parse_step(self, transition):
        ...
        if transition == 'S':
            ...
            raise ValueError(f'"{transition}" is an invalid transition')
        ### END YOUR CODE
...
def minibatch_parse(sentences, model, batch_size):
    ...
    while unfinished_parses:
        ...
        for i, partial_parse in enumerate(partial_parses_batch):
            partial_parse.parse_step(transitions[i])
            if len(partial_parse.stack) == 1 and not partial_parse.buffer:
                dependencies.append(unfinished_parses.pop(i).dependencies)
    ### END YOUR CODE
